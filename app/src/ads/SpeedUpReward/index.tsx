...
interface SpeedUpRewardProps {
  ...
  estimatedCompletionTime: any; // Firebase timestamp or Date
  disabled?: boolean;
}
...
export default function SpeedUpReward({
  ...
  const remainingMinutes = useMemo(() => {
    ...
    const completionTime = estimatedCompletionTime.toDate?.()
      ? estimatedCompletionTime.toDate().getTime()
      : new Date(estimatedCompletionTime).getTime();
    
    const distance = completionTime - currentTime.getTime();
    return Math.max(Math.ceil(distance / (1000 * 60)), 0);
